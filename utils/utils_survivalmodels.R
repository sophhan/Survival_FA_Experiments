...
################################################################################
#                    Adoption of survivalmodels/R/deephit.R
################################################################################
...
################################################################################
#                           survivalmodels/R/helpers_pycox.R
################################################################################
...
pycox_prepare_train_data <- function(x_train, y_train, frac = 0,
  ...
  y_train <- reticulate::tuple(
    y_train["time"]$values, #$astype(conv), # -------- Don't discretize time before label transform
  ...
  if (frac) {
    ...
    y_val <- reticulate::tuple(
      y_val["time"]$values, #$astype(conv), # -------- Don't discretize time before label transform
